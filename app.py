...
with col1:
    b_date = st.date_input("Дата рождения", value=pd.to_datetime("1979-06-07"))
with col2:
    b_time = st.time_input("Время рождения", value=pd.to_datetime("23:20").time())
with col3:
    city_input = st.text_input("Город рождения", value="Избербаш")

tz_list = ["Europe/Moscow", "Europe/Minsk", "Asia/Yekaterinburg", "Europe/London", "America/New_York", "UTC"]
b_tz = st.selectbox("Часовой пояс", tz_list, index=0)

if st.button("Рассчитать карту", type="primary"):
    try:
        # Локация
        geolocator = Nominatim(user_agent="astro_app_streamlit")
        location = geolocator.geocode(city_input)
        if not location:
            st.error("Город не найден! Попробуйте написать по-другому или по-английски.")
            st.stop()
            
        lat, lon = location.latitude, location.longitude
        st.success(f"📍 Локация: {location.address} (Широта: {lat:.2f}, Долгота: {lon:.2f})")
        ...
        ax.add_patch(plt.Circle((0, 0), 7, color='darkblue', fill=False, linewidth=1))
        ...
        for i in range(12):
            ...
            ax.text(8.5 * math.cos(a_text), 8.5 * math.sin(a_text), zodiac_signs[i][:3], ha='center', va='center', fontsize=10, fontweight='bold', color='navy')
        ...
        for i in range(12):
            ...
            ax.text(4.2 * math.cos(l_angle), 4.2 * math.sin(l_angle), roman_nums[i+1], fontsize=9, color='black', fontweight='bold')

        asc_angle, mc_angle = math.radians(ascmc[0]), math.radians(ascmc[1])
        ax.plot([0, 10 * math.cos(asc_angle)], [0, 10 * math.sin(asc_angle)], color='red', linewidth=2)
        ax.text(10.5 * math.cos(asc_angle), 10.5 * math.sin(asc_angle), 'ASC', color='red', fontweight='bold', ha='center', va='center')
        ax.plot([0, 10 * math.cos(mc_angle)], [0, 10 * math.sin(mc_angle)], color='red', linewidth=2)
        ax.text(10.5 * math.cos(mc_angle), 10.5 * math.sin(mc_angle), 'MC', color='red', fontweight='bold', ha='center', va='center')
        ...
        for (p1, lon1), (p2, lon2) in itertools.combinations(planet_positions.items(), 2):
            ...
            for t_angle, props in aspects_rules.items():
                if abs(diff - t_angle) <= props["orb"]:
                    ...
                    ax.plot([6.2 * math.cos(a1), 6.2 * math.cos(a2)], [6.2 * math.sin(a1), 6.2 * math.sin(a2)], color=props["color"], alpha=0.5, linewidth=1.5)

        for name, lon_p in planet_positions.items():
            angle = math.radians(lon_p)
            ax.plot(6.2 * math.cos(angle), 6.2 * math.sin(angle), 'o', color='crimson', markersize=9, zorder=5)
            ax.text(5 * math.cos(angle), 5 * math.sin(angle), name, ha='center', va='center', fontsize=9, zorder=6)

        # Вывод на экран
        col_img, col_tables = st.columns([1.2, 1])
        with col_img:
            st.pyplot(fig)
        with col_tables:
            st.subheader("Положение планет")
            st.dataframe(pd.DataFrame(results), use_container_width=True, hide_index=True)
            st.subheader("Куспиды Домов (Плацидус)")
